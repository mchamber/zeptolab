...
import orm from '@src/repos/MockOrm'; // Import the 'orm' object from the appropriate module
...
describe('FileRouter', () => {
  ...
  describe("Unit test saveFile", () => {
    ...
    it('should handle errors when saving a file', async () => {
      ...
      expect(response.body.error).toContain("File can't be read: invalidType/invalidVersion.json");
    });
  });
  ...
  describe("Integration test saveFile", () => {
    ...
    it('should return content null when invalid hash', async () => {
      ...
      const response = await superTestApp.post('/api/saveFileRpc').send(request);
      expect(response.status).toBe(200);
      expect(response.body).toEqual({ type, version, hash: jasmine.any(String), content: null });
    });
